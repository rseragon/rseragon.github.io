...
### TL;DR
> Duff's copy is a method of implementing array copying in C that relies on unusual syntactic sugar. Although it may not be as efficient as the standard copying algorithm, it is not commonly used in modern times. However, it is still considered an interesting approach that could assist programming novices in appreciating the beauty of C's syntax.
{: .prompt-info}
...
number of iterations we can do per loop. For a better intiution, look at the video below.
<center>
